...
pub fn generate_syntax() -> Result<()> {
    ...
    let ast_tokens_file = project_root().join("crates/abs_syntax/src/ast/generated/tokens.rs");
    let contents = generate_tokens(&ast)?;
    update(ast_tokens_file.as_path(), &contents)?;

    let ast_nodes_file = project_root().join("crates/abs_syntax/src/ast/generated/nodes.rs");
    let contents = generate_nodes(KINDS_SRC, &ast)?;
    update(ast_nodes_file.as_path(), &contents)?;
...
fn lower(grammar: &Grammar) -> AstSrc {
    ...
    res.tokens = vec!["Whitespace".into(), "Comment".into(), "String".into()];
    ...
    match rule {
        ...
        Rule::Labeled { label: l, rule } => {
            ...
            let manually_implemented = matches!(
                l.as_str(),
                "lhs" | "rhs" | "then_branch" | "else_branch" | "op" | "value"
            );
...
fn lower_comma_list(
    ...
    match repeat.as_slice() {
        [Rule::Token(_), Rule::Node(n)] if n == node => (),
        _ => return false,
    }
    let ty = grammar[*node].name.clone();
    let name = label
...
fn extract_enum_traits(ast: &mut AstSrc) {
    for enm in &mut ast.enums {
        ...
        let mut variant_traits = enm
            .variants
            .iter()
            .map(|var| nodes.iter().find(|it| &it.name == var).expect(var))
